...
### Commit-list timestamps (%cI)

For commit lists, use `git log --format='%h %cI %s'` — `%cI` is the committer
date in strict ISO 8601 WITH the timezone offset. The offset is the committing
machine's LOCAL timezone and must be read, not assumed: -05:00 in September is
CDT (America/Chicago), which is NOT the -04:00 (EDT) of America/New_York.
When asked to report timezone, report `git config --get log.date`, `date +%Z`,
and the system zone (`readlink /etc/localtime`) — do not change config and do
not silently convert. UTC conversion: python datetime arithmetic
(`local_dt - offset` with sign-aware hours/minutes); `gdate` may not exist.
