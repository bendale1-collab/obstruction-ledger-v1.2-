...
Never a silent halt, never a fabricated fill, never a verdict. A blocked source is
reported AS blocked with the reason; partial data is reported AS partial. The human
reads state.json and authors the gate. The script authors nothing.

RUN OFF-MACHINE (USPTO/OpenAlex must be reachable; the analysis box proxy blocks USPTO).

Signals (measured SEPARATELY, intersected — never ORed):
  C = citation death (OpenAlex)          P = patent lapse (USPTO maintenance fee)
Gate candidate = |P & C| >= 30. Script reports the number; human decides.

  python fetch_corpus.py --class her --citedeath-cutoff-year 2012 --out ./corpus_out

Patent source is a NotImplementedError by default. Wire ONE route before running:
  - Google Patents BigQuery (OAuth)
  - USPTO BDSS bulk XML download
  - Lens.org API

See prosecutor skill references/corpus-circularity-gate.md for source availability status.
"""
import argparse, json, sys, time, traceback, urllib.request, urllib.parse
from pathlib import Path

# ---- FROZEN (do not modify without re-registering) ----
GATE_N = 30
OPENALEX = "https://api.openalex.org/works"
CLASS_TOPIC = {"her": "T10030"}                 # HER electrocatalysts
CLASS_CPC   = {"her": ["C25B", "B01J"]}         # candidate subclasses; human confirms
CITEDEATH_MAXCITES = 2                          # <= this over life = "dead" (long tail)
# --------------------------------------------------------
...
def fetch_citation_death(topic, cutoff_year, out, state):
    ...
    try:
        while cursor:
            ...
            pages += 1
            state["C"]["pages_fetched"] = pages
            state["C"]["count"] = len(dead)
            emit(out, state)
        state["C"]["status"] = "COMPLETE"
    except Exception as e:
        state["C"]["status"] = "PARTIAL"
        state["C"]["blocker"] = str(e)
...
def fetch_patent_lapse(cpc_list, out, state):
    try:
        raise NotImplementedError(
            "Patent source not wired. Implement ONE reachable route: USPTO BDSS "
            "maintenance-fee bulk file, BigQuery patents.maintenance_fee_events, or Lens API. "
            "Do NOT default P to empty.")
    except Exception as e:
        state["P"]["status"] = "DATA_WALL"
        state["P"]["blocker"] = str(e)
        state["P"]["count"] = None
        emit(out, state)
        return None
...
def main():
    ...
    try:
        ...
        if P is None:
            ...
            inter = C & P
            state["intersection"] = {"count": len(inter),
                "meets_gate": len(inter) >= GATE_N}
        ...
        state["fatal"] = traceback.format_exc()
        emit(out, state)
        (out / "ERROR.txt").write_text(state["fatal"])
        log("FATAL \u2014 state.json + ERROR.txt written. No counts fabricated.")
        sys.exit(2)
